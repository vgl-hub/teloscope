# shellcheck shell=bash

# Axis C: boundary tests at threshold limits.

# ---- -l / --min-block-length, default 300 ----
fx th_len_below synthetic/th_len_below.fa \
   'chr_th_len_below=F:CCCTAAx49+L:2706' '-' \
   'type=none;anom=.;gran=;telo=0;labels=none;gaps=0' \
   '294 bp array is below -l 300 and is dropped'

fx th_len_exact synthetic/th_len_exact.fa \
   'chr_th_len_exact=F:CCCTAAx50+L:2700' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   '300 bp array is exactly -l and is kept: the comparison rejects on strict <'

fx th_len_above synthetic/th_len_above.fa \
   'chr_th_len_above=F:CCCTAAx51+L:2694' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   '306 bp array is above -l and is kept'

# ---- --terminal-tolerance, default 2000, in called bases ----
fx th_tol_exact synthetic/th_tol_exact.fa \
   'chr_th_tol_exact=L:2000+F:CCCTAAx100+L:2800' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   'Array starting exactly at --terminal-tolerance is still terminal'

fx th_tol_beyond synthetic/th_tol_beyond.fa \
   'chr_th_tol_beyond=L:2400+F:CCCTAAx100+L:2400' '-' \
   'type=none;anom=.;gran=;telo=0;labels=none;gaps=0' \
   'Array starting 2400 bases in is past --terminal-tolerance at either end'

# docs/parameters.md:91: tolerance counts called bases; leading Ns don't count against it.
fx th_tol_called_bases synthetic/th_tol_called_bases.fa \
   'chr_th_tol_called_bases=N:1000+L:1000+F:CCCTAAx100+L:2800' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=1' \
   'A leading 1000 bp gap does not push a telomere out of the terminal zone'

# ---- -d / --max-block-distance, default 500 ----
fx th_bridge_plain_exact synthetic/th_bridge_plain_exact.fa \
   'chr_th_bridge_plain_exact=F:CCCTAAx50+L:500+F:CCCTAAx50+L:2000' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   'A 500 bp plain-sequence run is exactly -d, so the two arrays merge into one block'

fx th_bridge_plain_beyond synthetic/th_bridge_plain_beyond.fa \
   'chr_th_bridge_plain_beyond=F:CCCTAAx50+L:506+F:CCCTAAx50+L:2000' '-' \
   'type=incomplete;anom=misassembly;gran=Pp;telo=1;labels=p;gaps=0' \
   'A 506 bp run exceeds -d, so the arrays stay separate and the inner one is extra'

fx th_bridge_gap_exact synthetic/th_bridge_gap_exact.fa \
   'chr_th_bridge_gap_exact=F:CCCTAAx50+N:500+F:CCCTAAx50+L:2000' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=1' \
   'A 500 bp N run is bridged for free, giving one gapped block'

fx th_bridge_gap_beyond synthetic/th_bridge_gap_beyond.fa \
   'chr_th_bridge_gap_beyond=F:CCCTAAx50+N:506+F:CCCTAAx50+L:2000' '-' \
   'type=incomplete;anom=misassembly;gran=Pp;telo=1;labels=p;gaps=1' \
   'A 506 bp N run exceeds -d and ends the block'

# ---- --min-its-length, default 100 ----
fx th_its_below synthetic/th_its_below.fa \
   'chr_th_its_below=F:CCCTAAx100+L:2400+R:TTAGGGx16+L:2400+R:TTAGGGx100' '-i -t 1000' \
   'type=t2t;anom=.;gran=PQ;telo=2;labels=pq;gaps=0;its=0' \
   'A 96 bp interstitial array is below --min-its-length and is not reported'

fx th_its_exact synthetic/th_its_exact.fa \
   'chr_th_its_exact=F:CCCTAAx100+L:2400+R:TTAGGGx17+L:2400+R:TTAGGGx100' '-i -t 1000' \
   'type=t2t;anom=.;gran=PQ;telo=2;labels=pq;gaps=0;its=1' \
   'A 102 bp interstitial array clears --min-its-length'

# minCanonicalCount threshold (REG-003).
fx th_its_canon_below synthetic/th_its_canon_below.fa \
   'chr_th_its_canon_below=F:CCCTAAx100+L:2400+R:TTAGGGx3+L:2400+R:TTAGGGx100' \
   '-i -t 1000 --min-its-length 18' \
   'type=t2t;anom=.;gran=PQ;telo=2;labels=pq;gaps=0;its=0' \
   'Three canonical matches are below the hardcoded minCanonicalCount of 4'

fx th_its_canon_exact synthetic/th_its_canon_exact.fa \
   'chr_th_its_canon_exact=F:CCCTAAx100+L:2400+R:TTAGGGx4+L:2400+R:TTAGGGx100' \
   '-i -t 1000 --min-its-length 18' \
   'type=t2t;anom=.;gran=PQ;telo=2;labels=pq;gaps=0;its=1' \
   'Four canonical matches are exactly minCanonicalCount and are reported'

fx th_its_canon_above synthetic/th_its_canon_above.fa \
   'chr_th_its_canon_above=F:CCCTAAx100+L:2400+V:TTAGGAx2+R:TTAGGGx1+V:TTAGGAx2+R:TTAGGGx1+V:TTAGGAx2+R:TTAGGGx1+V:TTAGGAx2+R:TTAGGGx1+V:TTAGGAx2+R:TTAGGGx1+V:TTAGGAx2+L:2400+R:TTAGGGx100' \
   '-i -t 1000' \
   'type=t2t;anom=.;gran=PQ;telo=2;labels=pq;gaps=0;its=1' \
   'Five non-adjacent canonical repeats among variants, 102 bp: clears --min-its-length 100 and minCanonicalCount 4'

# Strand label thresholds 666/333 per mille (REG-006).
fx th_strand_2fwd_1rev synthetic/th_strand_2fwd_1rev.fa \
   'chr_th_strand_2fwd_1rev=M:CCCTAACCCTAATTAGGGx40+L:2800' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   'Two forward per one reverse is 66.67%, above the 666 threshold: strand p'

fx th_strand_1fwd_2rev synthetic/th_strand_1fwd_2rev.fa \
   'chr_th_strand_1fwd_2rev=M:CCCTAATTAGGGTTAGGGx40+L:2800' '-' \
   'type=incomplete;anom=balanced_p;gran=P~;telo=1;labels=p;gaps=0' \
   'One forward per two reverse is 33.33%, not below the 333 threshold: strand b, not q'

# ---- strand sweep at 0, 333, 666, 1000 per mille: exact fractions ----
fx th_strand_0 synthetic/th_strand_0.fa \
   'chr_th_strand_0=M:TTAGGGx100+L:2400' '-' \
   'type=incomplete;anom=discordant_p;gran=P*;telo=1;labels=p;gaps=0' \
   '100 reverse matches, 0 forward: forward share is 0 per mille exactly'

fx th_strand_333 synthetic/th_strand_333.fa \
   'chr_th_strand_333=M:TTAGGGTTAGGGCCCTAAx333+M:TTAGGGx1+L:2400' '-' \
   'type=incomplete;anom=balanced_p;gran=P~;telo=1;labels=p;gaps=0' \
   '333 forward matches of 1000 total (333x "R R F" plus one appended R): exactly 333 per mille'

fx th_strand_666 synthetic/th_strand_666.fa \
   'chr_th_strand_666=M:CCCTAACCCTAATTAGGGx166+M:CCCTAATTAGGGx1+L:2400' '-' \
   'type=incomplete;anom=balanced_p;gran=P~;telo=1;labels=p;gaps=0' \
   '333 forward matches of 500 total (166x "F F R" plus one appended "F R"): exactly 666 per mille'

fx th_strand_1000 synthetic/th_strand_1000.fa \
   'chr_th_strand_1000=M:CCCTAAx100+L:2400' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   '100 forward matches, 0 reverse: forward share is 1000 per mille exactly'

# ---- -y / --min-block-density, default 0.5 ----
fx th_density_exact synthetic/th_density_exact.fa \
   'chr_th_density_exact=M:CCCTAAACGATCx50+L:2400' '-' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   '6 bp match against 6 bp filler is exactly -y 0.5 and is kept on the tie'

fx th_density_below synthetic/th_density_below.fa \
   'chr_th_density_below=M:CCCTAAACGATCGACTx50+L:2400' '-' \
   'type=none;anom=.;gran=;telo=0;labels=none;gaps=0' \
   '6 bp match against 10 bp filler is 37.5%, below -y 0.5'

# ---- --min-block-counts, default 2 ----
fx th_counts_below synthetic/th_counts_below.fa \
   'chr_th_counts_below=F:CCCTAAx1+L:2994' '-l 6' \
   'type=none;anom=.;gran=;telo=0;labels=none;gaps=0' \
   'A single match is below --min-block-counts 2'

fx th_counts_exact synthetic/th_counts_exact.fa \
   'chr_th_counts_exact=F:CCCTAAx2+L:2988' '-l 6' \
   'type=incomplete;anom=.;gran=P;telo=1;labels=p;gaps=0' \
   'Two matches are exactly --min-block-counts and are kept'
