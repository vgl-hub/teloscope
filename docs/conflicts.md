...
zone is the smaller of the two. Blocks within `-d` (`1000`) chain into one telomere; `-k`,
`-d`, `--terminal-tolerance` and `-t` each do one job.
...
pieces within `-d` chain into one row (`fragmented_p`/`fragmented_q` when
